...
describe('resource calibrate', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.piScoringSystem.calibrate.create({
      scoring_system: {
  ...
  test.skip('create: required and optional params', async () => {
    const response = await client.piScoringSystem.calibrate.create({
      scoring_system: {
        description: "Write a children's story communicating a simple life lesson.",
        name: 'Sample Scoring System',
        dimensions: [
  ...
  test.skip('messages', async () => {
    const responsePromise = client.piScoringSystem.calibrate.messages('job_id');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('messages: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.piScoringSystem.calibrate.messages('job_id', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Withpi.NotFoundError);
  });
});
