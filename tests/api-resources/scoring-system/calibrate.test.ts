...
describe('resource calibrate', () => {
  ...
  test.skip('launch: only required params', async () => {
    const responsePromise = client.scoringSystem.calibrate.launch({
      scoring_system: {
  ...
  test.skip('launch: required and optional params', async () => {
    const response = await client.scoringSystem.calibrate.launch({
      scoring_system: {
        description: "Write a children's story communicating a simple life lesson.",
        name: 'Sample Scoring System',
        dimensions: [
  ...
  test.skip('status', async () => {
    const responsePromise = client.scoringSystem.calibrate.status('job_id');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('status: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.scoringSystem.calibrate.status('job_id', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Withpi.NotFoundError);
  });
});
