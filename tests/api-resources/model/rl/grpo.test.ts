...
describe('resource grpo', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.model.rl.grpo.create({
  ...
  test.skip('create: required and optional params', async () => {
    const response = await client.model.rl.grpo.create({
      base_rl_model: 'LLAMA_3.2_3B',
      examples: [{ llm_input: 'Tell me something different' }],
      learning_rate: 0.000005,
  ...
  test.skip('load: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.model.rl.grpo.load('job_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(
      Withpi.NotFoundError,
    );
  });
  ...
  test.skip('messages', async () => {
    const responsePromise = client.model.rl.grpo.messages('job_id');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
